...
export const Boton = ({ product }) => {
  ...
  const handleIncrement = () => {
    ...
    dispatch({ type: 'add', payload: { ...product, cantida: cantidad + 1 } });
  };
    ...
    <div className={`w-36 border-solid border-gray-300 rounded-full p-2 relative mx-auto ${cantidad <= 0 ? 'bg-[#fff]' : ' bg-[#c3410f]'}`}>
      {
        cantidad === 0
          ...
          : <div className="flex justify-around items-center">
            <button className="text-white text-lg" onClick={handleDecrement}>-</button>
            <span className="text-white text-lg">{cantidad}</span>
            <button className="text-white text-lg" onClick={handleIncrement}>+</button>
          </div>
      }
    </div>
  );
